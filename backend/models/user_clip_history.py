...
class UserClipHistory(Base):
    ...
    chat_messages = Column(Text, nullable=True)  # JSON array of {role, content, timestamp}
    
    # Timeline of all edits (stored as JSON array)
    edit_history = Column(Text, nullable=True)  # JSON array of {action, timestamp, before, after}
